...
var largest = firebase.database().ref("UID/");
count = 0;
...
var newSID = firebase.database().ref().push().key;
var updates={};
updates['UID/' + newSID] = data.StaffID;
    largest.child(count).once('value').then(function(snapshot){
  if(snapshot.exists()){
    alert('Already exists: '+ count)
  }
})
